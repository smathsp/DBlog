...
const HOST = process.env.INDEXNOW_HOST || 'blog.smathsp.com';
...
async function main() {
	...
	if (args[0] === 'sitemap') {
		...
		urls = await fetchUrlsFromSitemap(`https://blog.smathsp.com/sitemap.xml`);
		console.log(`   Found ${urls.length} URLs`);
	} else {
		// Use provided URLs
		urls = args.map((url) => {
			// Ensure URL has protocol
			return url.startsWith('http') ? url : `https://blog.smathsp.com${url.startsWith('/') ? '' : '/'}${url}`;
		});
	}
